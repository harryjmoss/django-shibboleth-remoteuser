...
SHIB_ATTRIBUTE_MAP = getattr(
    settings, 'SHIBBOLETH_ATTRIBUTE_MAP', default_shib_attributes)
...
SHIB_REMOTE_USER_HEADER = getattr(
    settings, 'SHIBBOLETH_REMOTE_USER_HEADER', 'REMOTE_USER')

LOGIN_URL = getattr(settings, 'LOGIN_URL', None)

if not LOGIN_URL:
    raise ImproperlyConfigured("LOGIN_URL is required. Specify in settings.py")
...
# LOGOUT_REDIRECT_URL specifies a default logout page that will always be
# used when users logout from Shibboleth.
LOGOUT_REDIRECT_URL = getattr(settings, 'SHIBBOLETH_LOGOUT_REDIRECT_URL', None)

# Name of key.  Probably no need to change this.
LOGOUT_SESSION_KEY = getattr(
    settings, 'SHIBBOLETH_FORCE_REAUTH_SESSION_KEY', 'shib_force_reauth')
